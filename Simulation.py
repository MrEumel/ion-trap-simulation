            ...
            for frequ in range(len(f_0)):
                for fric in range(len(beta_0)):
                    ...
                    if dimensions == 3:
                        ...
                        #z_fix = np.random.choice(initial_pos[:, 2], size=n, replace=False) # z-values need to be randomly picked (otherwise ions start in a tilted plane, which works fine too)
...
if evaluate_simulation_results == 1:
    ...
    if duty_cycle_density_diagram == 1:
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            plt.plot(results_stable[14, :], results_stable[9, :], 'bo')
            plt.plot(results_unstable[14, :], results_unstable[9, :], 'ro')
            # plt.hlines(1e-6, min_results_density, max_results_density)
            plt.grid()
            # plt.ylim(0, 1)
            #plt.xlim(min_results_density, max_results_density)
            plt.ylabel('ion amount in beam radius')
...
if evaluate_complete_results == 1:
    ...
    if radius_diagram == 1:
        ...
        plt.xlabel('q value')
        plt.ylabel('cloud radius')
        plt.savefig(evaluation_dir + 'complete_results_R_q.png')
        if show_phase_diagrams == 1:
            plt.show()
        plt.clf()
        ...
        plt.xlabel('beta value')
        plt.ylabel('cloud radius')
        plt.savefig(evaluation_dir + 'complete_results_R_beta.png')
        if show_phase_diagrams == 1:
            plt.show()
        plt.clf()
    ...
    if n_q_diagram == 1:
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            plt.xlabel('q value')
            plt.ylabel('ion amount in beam area')
            plt.savefig(evaluation_dir + 'complete_results_n_beam_q.png')
            if show_phase_diagrams == 1:
                plt.show()
    ...
    if n_beta_diagram == 1:
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            plt.xlabel('beta value')
            plt.ylabel('ion amount in beam area')
            plt.savefig(evaluation_dir + 'complete_results_n_beam_beta.png')
            if show_phase_diagrams == 1:
                plt.show()
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            if Nq_beta_filter == 1:
                ...
                if Nq_q_filter == 1:
                    ...
                    print('Critical N value between: ' + str(max_stable_N) + ' and ' + str(min_unstable_N))
                    plt.vlines(N_crit, 0, 1)
                plt.savefig(evaluation_dir + 'complete_results_N_q_betamin' + str(Nq_beta_filter_min) + '_betamax' + str(Nq_beta_filter_max) + '.png')
            else:
                plt.savefig(evaluation_dir + 'complete_results_N_q.png')
            if show_phase_diagrams == 1:
                plt.show()
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            if betaq_N_filter == 1:
                ...
                if betaq_beta_filter == 1:
                    ...
                    print('Critical q value between: ' + str(max_stable_q) + ' and ' + str(min_unstable_q))
                    plt.hlines(q_crit, betaq_beta_filter_min, betaq_beta_filter_max)
                plt.savefig(evaluation_dir + 'complete_results_beta_q_Nmin' + str(betaq_N_filter_min) + '_Nmax' + str(betaq_N_filter_max) + '.png')
            else:
                plt.savefig(evaluation_dir + 'complete_results_beta_q.png')
            if show_phase_diagrams == 1:
                plt.show()
        ...
        if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
            ...
            if betacycle_N_filter == 1:
                ...
                if betacycle_beta_filter == 1:
                    ...
                    plt.hlines(pulse_crit, betacycle_beta_filter_min, betacycle_beta_filter_max)
                plt.savefig(evaluation_dir + 'complete_results_beta_duty_cycle_Nmin' + str(betaq_N_filter_min) + '_Nmax' + str(
                    betaq_N_filter_max) + '.png')
            else:
                plt.savefig(evaluation_dir + 'complete_results_beta_duty_cycle.png')
            if show_phase_diagrams == 1:
                plt.show()
            ...
            if len(results_stable[0, :]) and len(results_unstable[0, :]) > 0:
                ...
                if cycle_density_N_filter == 1:
                    plt.title('Filtered for ' + str(cycle_density_N_filter_min) + ' =< N =< ' + str(
                        cycle_density_N_filter_max))
                    plt.savefig(
                        evaluation_dir + 'complete_results_duty_cycle_density_Nmin' + str(cycle_density_N_filter_min) + '_Nmax' + str(
                            cycle_density_N_filter_max) + '.png')
                if cycle_density_beta_filter == 1:
                    plt.title('Filtered for ' + str(cycle_density_beta_filter_min) + ' =< beta =< ' + str(
                        cycle_density_beta_filter_max))
                    plt.savefig(evaluation_dir + 'complete_results_duty_cycle_density_betamin' + str(
                        cycle_density_beta_filter_min) + '_betamax' + str(
                        cycle_density_beta_filter_max) + '.png')
                else:
                    plt.savefig(evaluation_dir + 'complete_results_duty_cycle_density.png')
                if show_phase_diagrams == 1:
                    plt.show()
